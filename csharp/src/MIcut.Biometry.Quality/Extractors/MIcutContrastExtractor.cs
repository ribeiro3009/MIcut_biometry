...
public sealed class MIcutContrastExtractor : IMIcutQualityExtractor
{
    public string Nome => "MIcutContrast";
    public int ScoreMaximo => 128;
    ...
    public int ExtrairQualidade(int posicaoGrid, byte[]? imagemBmpBytes, bool ehAmputado)
    {
        ...
        try
        {
            ...
            Cv2.MeanStdDev(gray, out _, out var stddev, mask);
            double sd = stddev.Val0;
            int score = (int)Math.Round(sd, MidpointRounding.AwayFromZero);
